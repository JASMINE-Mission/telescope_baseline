...
import numpy as np
import tqdm
from telescope_baseline.mapping.aperture import lb_detector_unit, four_square_convexes, inout_four_square_convexes, \
    ang_detector_unit, lb2ang, ang2lb


def ditheringmap(l_center, b_center, PA_deg, dithering_width_mm, Ndither, width_mm=22.4, each_width_mm=19.52,
                 EFL_mm=4370.0, left=0.0, top=0.0):
    ...
    for i in tqdm.tqdm(range(0, Ndither[0])):
        for j in range(0, Ndither[1]):
            ...
            convexes = four_square_convexes(ang_detector_unit("B", dithering_width * j, pos, PA, width), PA, width,
                                            each_width)
...
def fillgap_large_frame(l_center, b_center, PA_deg, width_mm=22.4, each_width_mm=19.52, EFL_mm=4370.0, left=0.0,
                        top=0.0):
    """fill gap dithering
    ...
    """
    pos = []
    pos1 = large_frame(l_center, b_center, PA_deg, width_mm=width_mm, each_width_mm=each_width_mm, EFL_mm=EFL_mm,
                       left=0.125 + left, top=-0.125 / 2.0 + top)
    pos.append(pos1)
    pos2 = large_frame(l_center, b_center, PA_deg, width_mm=width_mm, each_width_mm=each_width_mm, EFL_mm=EFL_mm,
                       left=-0.125 + left, top=-0.125 / 2.0 + top)
    pos.append(pos2)
    pos3 = large_frame(l_center, b_center, PA_deg, width_mm=width_mm, each_width_mm=each_width_mm, EFL_mm=EFL_mm,
                       left=0.125 + left, top=0.125 / 2.0 + top)
    pos.append(pos3)
    pos4 = large_frame(l_center, b_center, PA_deg, width_mm=width_mm, each_width_mm=each_width_mm, EFL_mm=EFL_mm,
                       left=-0.125 + left, top=0.125 / 2.0 + top)
    pos.append(pos4)
